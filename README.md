...
4. **Retry Draft** regenerates only the active draft with its saved inputs. **Retry Draft with Different Seed** first assigns and persists a different random 32-bit seed, then queues the same retry path. Each successful retry retains the prior artifact, preview, latent, and frozen generation inputs as another selectable draft take.
...
LongCaster can create a same-resolution, one-pass H3 derivative after an accepted card. The accepted MMH3 master is published and remains immutable; the derivative is stored separately and is used only as an optional source for the next direct-MMH3 continuation.

Connect an explicit schedule to the project node's `refine_sigmas` input. LongCaster never silently reuses the first-pass/PDD `SIGMAS`. `refine_model` is optional and defaults to the first-pass `MODEL`; the refine sampler defaults to Euler and the seed policy defaults to inherit. The initial implementation always samples the joint audio/video latent, reconstructs both streams' continuation mask for continued cards, and rejects the derivative if any hard-protected prefix value changes.

Choose the synchronized **Off**, **Auto**, or **Manual** radio option in Studio. Off suppresses all refinement. Auto refines every newly accepted card. Manual uses each editable card's **Refine enabled** checkbox. Acceptance always commits the immutable master first; a refine failure leaves it selected. Accepted cards expose **Generate Refine** or **Regenerate Refine**, except that Off keeps the action disabled. Unpublishing a card removes all of that card's refine files and metadata. See [Stage 3A continuation refine](docs/stage-3a-continuation-refine.md) for the stored diagnostics and validation boundary.
...
**Duplicate Project** creates a complete independent copy under a new project name. It copies prompts, settings, cards, masters, drafts, refine derivatives, previews, and anchors, then rewrites the embedded LongCaster project name in every referenced MMH3 archive and records new artifact hashes. Duplication is refused while an operation is pending or when a recorded artifact is missing or corrupt; the destination is staged and appears in Studio only after the full copy validates.

The Project tab contains **Project LoRA Activation Words**. LongCaster preserves the entered case and punctuation, then injects the trimmed text at runtime immediately after `subject_definitions:` for every newly generated card. Saved card prompt sections are not modified. The activation text is included in the effective prompt, recipe, and generation fingerprint. Changing it while a draft exists marks that draft dirty and requires Retry. This feature only supplies trigger words; the corresponding LoRA must still be applied to the connected model graph.
...
Studio is divided into **Card** and **Project** tabs. Card contains the timeline, structured prompt editor, card preview, and generation actions. Project contains saved identity anchors and project reference inspection. Stop Generation and the execution panel remain available in both tabs.

The Card Preview sidebar lists every successful **Draft Take** for that card. **View** previews an alternative without changing the active draft. The collapsible **Prompt Diff** compares that viewed take's immutable archived prompt to the current working prompt, using removed and added code-diff lines, and updates while the working prompt is edited. **Take Settings / Generation Setup** shows the frozen canvas, sampler, scheduler, exact sigmas, `match`/`max` REF2VA sizing, references, diffusion/checkpoint loaders, LoRAs with their strengths, and every other node on the submitted MODEL path. Its frozen recipe also retains a compact snapshot of all submitted upstream nodes feeding the first-pass model, CLIP, VAEs, sigmas, and reference packet. **Select** restores that take's exact prompt, seed, duration, reference sizing, recipe, references, latent archive, and preview; selection is available only while the active card is a draft. **Accept Draft** publishes the selected take. Individual unselected takes, or all unselected takes together, can be permanently deleted. The selected take cannot be deleted, and accepted cards must first be unpublished before their selection can change.

Rendered cards are marked **VALIDATED**. **Invalidate Render** is available only for the active final card, since later cards may depend on its MMH3 latent and anchors. After confirmation it permanently removes that card's master, draft takes, previews, refinements, anchors, and superseded publications while retaining the prompt, card identity, duration, seed, and continuation choice. The card becomes **INVALIDATED** and can be generated again with the currently connected model/setup. Accepted artifact numbers are not reused.

The Project tab provides the project aspect-ratio and megapixel controls. While any render remains, they are disabled with **“To change resolution, invalidate all cards first.”** **Invalidate All Renders** warns that all renders will be lost, then deletes every card's associated render lineage while retaining all prompts and card definitions. It unlocks resolution editing and makes Card 1 active. Regenerate and accept the preserved cards in order; each acceptance automatically activates the next invalidated card.

**Duplicate and Invalidate Project** creates a new definition-only project for rerendering. It copies project settings, the complete card timeline, prompts, durations, seeds, and continuation choices, but does not copy MMH3 archives or other render artifacts. The source project is unchanged; the new project opens with every card invalidated, Card 1 active, and resolution editing unlocked.

The Project identity manager shows each checkpoint's image, label, subject, source card and frame, scope, enabled state, and active subject binding. Checkpoints currently in use have a green background and an explicit **IN USE FOR** label. **Clear** only removes the active binding; **Delete** unbinds the checkpoint, removes its saved record, and permanently deletes its project-owned image after confirmation. To create another checkpoint, select any accepted source card, scrub its project preview, choose the subject and scope, and use the selected frame. A missing source preview can be generated from the same panel through the workflow's Identity Anchor node.

Accepted cards show **Unpublish to Draft**. It is enabled for the active accepted tail and reopens that card as a retryable draft while retaining its immutable accepted master in publication history. On an earlier accepted card it remains visible but disabled, explaining that a later unaccepted draft must be removed first. **Remove Draft Card** appears for the active appended `EMPTY`, `DRAFT`, or `FAILED` tail, including a card that was previously accepted and unpublished. After confirmation it permanently removes that card's current draft and any superseded publications, derivatives, anchors, and previews, then makes the previous accepted card active. The previous card can then be unpublished to edit its prompt. The first card cannot be removed because there is no previous accepted card to return to.
...
Each Card has a **REF2VA reference image size** selector. `match` sizes references to the card generation canvas; `max` uses the upstream maximum-reference behavior. The choice is persisted per card, appended cards inherit it, and Retry automatically uses the displayed card value. Changing it after a successful draft marks that draft dirty until Retry creates a matching take.

**Retry Draft with Different Seed** changes the editable card's seed to a new random unsigned 32-bit value, saves it visibly in the card and project manifest, and then performs a normal Retry. The previous draft remains authoritative until the replacement generation commits successfully and is then retained as an unselected take.
