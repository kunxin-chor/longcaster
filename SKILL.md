...
- `longcaster/prompt_sections.py` owns the canonical six-section prompt schema, exact deterministic assembly/hash, legacy parser, default inheritance, copy provenance, and edit provenance. Keep prompt assembly out of routes, nodes, and frontend callbacks.
...
- `longcaster/routes.py` exposes lightweight server APIs for the Cards workspace and identity picker: project/card discovery, revision-checked structured edits/copies, project-owned video playback, and anchor controls. Refer to it when either interface cannot load state, play a registered preview, save a card, detect a stale revision, or update anchor state without queueing generation.
...
- `web/longcaster.js` adds the full-size Cards workspace plus ComfyUI project/identity controls. Refer to it for six-section navigation, autosave, section-copy UX, project/card inspection, previews, queue behavior, command IDs, interrupt/unlock behavior, dynamic titles, or widget interactions. Keep validation, assembly, provenance, and state transitions in Python because workflows can run through the API without this frontend.
...
- `tests/test_prompt_sections.py` covers exact six-section parsing/assembly, hashing, whitespace, inheritance, and provenance. Run it after any structured-prompt change.
